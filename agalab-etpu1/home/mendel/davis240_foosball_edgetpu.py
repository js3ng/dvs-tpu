...
device.set_bias_from_json("/home/mendel/pyaer/scripts/configs/davis240c_config.json")
...
#            davis_image += chunk
            ...
            latency, track = track_engine.RunInference(track_input)
            
            track = np.reshape(np.array(track), (180,240))
            #print(track)
            coord= np.unravel_index(np.argmax(track[:,:], axis = None),track[:,:].shape)
...
#                print("Ball Coordinates = ", coord)
                ...
                if first_flag:
                    ...
                    latency, predict = predict_engine.RunInference(np.reshape(inp, (4,)))
...
#            print("Number of events:", num_pol_event, "Number of Frames:", frames.shape)
        else:
            pass
