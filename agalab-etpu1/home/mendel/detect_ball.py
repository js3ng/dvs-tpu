...
while True:
    try:
        ...
        if data is not None:
            ...
            print(latency)
#            print(int(raw_result[1]),int(raw_result[0]))
#            raw_result = int(raw_result)
            message = np.array((raw_result[1],raw_result[0]), dtype = 'int').tobytes() #encode('ascii')# + str(int(raw_result[1])).encode('ascii')
